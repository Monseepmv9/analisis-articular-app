...
def process_video_multijoint(video_path, configs, target_fps, preview_placeholder, progress_bar, progress_text, start_sec, end_sec):
    ...
    while frames_processed <= total_frames:
        ...
        if frames_processed % frame_step == 0:
            ...
            h_f, w_f = frame_analyzed.shape[:2]
            scale = 480 / float(w_f)
            dim = (480, int(h_f * scale))
...
def build_pdf_report(sessions):
    ...
    for s in sessions:
        ...
        pdf.cell(60, 8, "Máximo Alcanzado", border=1, fill=True)
        pdf.cell(60, 8, "Velocidad Máxima", border=1, fill=True, ln=True)
        
        pdf.set_font("Helvetica", "", 10)
        pdf.cell(60, 8, f"{s['max']}°", border=1)
...
if st.session_state.history:
    ...
    if st.button("Guardar temporalmente"):
        st.session_state.sessions.append({
            ...
            "max": round(max_angles_session[0], 1), 
            "vel": round(max_vels_session[0], 1),
